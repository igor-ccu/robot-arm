...
class TestWholeModule(unittest.TestCase):
    def test(self):
        objects = ('green_circle', 'blue_circle')
        test_images = glob.glob('*.jpg')
        with open("test.p", 'rb') as f:
            test_dictionary = pickle.load(f)
        for fn in test_images:
            test_image = cv2.imread(fn, 1)
            test_image = cv2.resize(test_image, (640, 360))
            for obj in objects:
                Pos, Pro = detector(test_image, obj)
                toCompareTo = test_dictionary[fn+obj]
                toTest = (hash(frozenset(Pos)),hash(Pro))
                self.assertEquals(toCompareTo, toTest)
