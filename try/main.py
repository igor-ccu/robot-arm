...
def cap():
    ...
    return cv2.undistort(image, mtx, dist, None, cameramtx)
...
def calib():
    ...
    global cameramtx = np.copy(newcameramtx) 
    return newcameramtx


def markers_cycle_horizontal(start=30, end=50, step=10, fct=display, threshold = 90, max_counts = 2):
    ...
    * angle are the angles of robot joints (ready to feed into cmd.mv_deg(angle))
    ...
    while detect_blue or detect_green:
        for angle in range(start, end+step, step):
            ...
            cv2.imshow("ueye", img)
            cv2.waitKey(100)
            if not (detect_blue and detect_green):
                break
        count += 1
        if blue_pos is None:
